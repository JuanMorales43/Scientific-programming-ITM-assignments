...
"""
Training and evaluation of an SVM classifier using features selected by LASSO.

This script:
- Loads the reduced CSV generated by the `LASSO.py` module.
- Uses only the selected radiomic features and the
  identification/label columns.
- Performs the partition into training, validation, and test sets.
- Standardizes the features before training the SVM.
- Fits the SVM model with the hyperparameters configured in the script.
- Calculates classification metrics and AUC-ROC on the different sets.
- Performs bootstrap resampling to obtain confidence intervals for the
  metrics (including AUC-ROC).
- Save the figures and reports needed to compare with the version
  without LASSO selection.

Its objective is to study the effect of feature selection on the
performance of SVM.
"""
...
# Coinfidence Intervals via Bootstrap
