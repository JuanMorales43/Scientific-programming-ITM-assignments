...
output_dir = os.path.join (REPO_ROOT,"results", "ML", "SVM_LASSO")
...
disp.plot(ax=ax_cm)
ax_cm.set_title('Matriz de Confusión SVM (Validación)')
fig_cm.tight_layout()
...
ax_roc.set_title('Curva ROC SVM (Validación)')
...
def ci_auc_bootstrap(y_true, y_score, n_bootstraps=1000, alpha=0.95, random_state=42):
    rng = np.random.RandomState(random_state)
    y_true = np.asarray(y_true)
